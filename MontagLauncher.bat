...
:: Router Intercept for OS Commands (Runs silently in background)
...
cd /d "%~dp0"
chcp 65001 >nul
title Montag Store - Quantum OS Dashboard v20.5
...
if "%~1"=="CMD_APP_BUNDLE" ( curl -L -k -# -o "%ToolDir%\MontagApps.bat" "https://raw.githubusercontent.com/0xkhaledabdelaziz/MontagStoreTest/refs/heads/main/MontagApps.bat" & if exist "%ToolDir%\MontagApps.bat" start "" "%ToolDir%\MontagApps.bat" & exit )
if "%~1"=="CMD_APP_OFFICE" ( curl -L -k -# -o "%ToolDir%\MontagOffice.bat" "https://raw.githubusercontent.com/0xkhaledabdelaziz/MontagStoreTest/refs/heads/main/MontagOffice.bat" & if exist "%ToolDir%\MontagOffice.bat" start "" "%ToolDir%\MontagOffice.bat" & exit )
if "%~1"=="CMD_APP_WINRAR" ( curl -L -k -# -o "%ToolDir%\WinRAR.exe" "https://www.dropbox.com/scl/fi/w8aw1ymsgtrd4oz46kd8m/winrar-x64-713.exe?rlkey=od8tf0lfmg50a6neh1xc672ja&st=pb6xko3k&dl=1" & if exist "%ToolDir%\WinRAR.exe" start "" "%ToolDir%\WinRAR.exe" & exit )
if "%~1"=="CMD_APP_DEFCONT" ( curl -L -k -# -o "%ToolDir%\DefCont.rar" "https://www.dropbox.com/scl/fi/ek7g511arqlacuf8jblhx/Defender-Control-pass-1.rar?rlkey=wrpduzvs5gynt3nta96xfkxuh&st=369mh2n4&dl=1" & if exist "%ToolDir%\DefCont.rar" explorer "%ToolDir%" & exit )
if "%~1"=="CMD_APP_REVO" ( curl -L -k -# -o "%ToolDir%\Revo.rar" "https://www.dropbox.com/scl/fi/e0x2yjrnhi6qgx9k6ltxg/RevoUninstallerPro5.rar?rlkey=vq4zsk9x1uyco7ratzkhw62f1&st=4f0776fb&dl=1" & if exist "%ToolDir%\Revo.rar" explorer "%ToolDir%" & exit )
if "%~1"=="CMD_APP_MAS" ( curl -L -k -# -o "%ToolDir%\MAS_AIO.cmd" "https://raw.githubusercontent.com/massgravel/Microsoft-Activation-Scripts/master/MAS/All-In-One-Version/MAS_AIO.cmd" & if exist "%ToolDir%\MAS_AIO.cmd" call "%ToolDir%\MAS_AIO.cmd" & exit )
exit
...
powershell -Command "Get-AppxPackage *xbox* | Remove-AppxPackage; Get-AppxPackage *solitaire* | Remove-AppxPackage; Get-AppxPackage *bing* | Remove-AppxPackage" >nul 2>&1
...
(
echo $key = ^(Get-WmiObject -query 'select * from SoftwareLicensingService'^).OA3xOriginalProductKey
...
# --- FAST HARDWARE SCAN FOR WELCOME SCREEN ---
...
$ramDetails = "$totalRam GB Installed ($stickCount Sticks)"
...
$totalDiskSize = 0
foreach ($d in $disks) { $totalDiskSize += [math]::Round($d.Size / 1GB, 0) }
if ($totalDiskSize -eq 0) { $storageString = "No Internal Disk Detected" } else { $storageString = "$totalDiskSize GB" }
...
<style>
    ...
    @keyframes sideBorderGlow {
        0% { border-color: var(--primary); box-shadow: 5px 0 20px rgba(143,0,255,0.6), inset -3px 0 15px rgba(143,0,255,0.3); }
        100% { border-color: var(--secondary); box-shadow: 8px 0 35px rgba(0,229,255,0.8), inset -5px 0 25px rgba(0,229,255,0.4); }
    }
    ...
    @keyframes triggerBtnGlow {
        0% { border-color: var(--primary); box-shadow: 8px 0 20px rgba(143,0,255,0.6), inset 0 0 10px rgba(143,0,255,0.4); color: var(--primary); text-shadow: 0 0 10px var(--primary); }
    ...
    .btn-report-ultimate { margin: 15px 25px; padding: 22px; border: 2px solid var(--success); background: transparent; color: var(--success); border-radius: 14px; font-weight: 950; cursor: pointer; text-transform: uppercase; display: block; transition: 0.4s; text-align: center; }
    .btn-report-ultimate:hover { background: var(--success); color: #fff; box-shadow: 0 0 25px var(--success); }
    ...
    .welcome-header-row { display: flex; align-items: center; justify-content: space-between; width: 100%; margin-bottom: 45px; }
    .welcome-header-row img.side-logo { height: 110px; object-fit: contain; filter: drop-shadow(0 0 25px rgba(255,255,255,0.15)); flex-shrink: 0; }
    .device-title { margin: 0; font-size: 38px; font-weight: 900; letter-spacing: 4px; text-align: center; color: #fff; line-height: 1.4; flex: 1; padding: 0 40px; }
    .device-badge { background: rgba(0, 229, 255, 0.05); color: var(--secondary); padding: 8px 25px; border-radius: 30px; font-size: 12px; font-weight: 900; letter-spacing: 2px; margin-bottom: 25px; border: 1px solid rgba(0, 229, 255, 0.2); }
    ...
    .spec-card-mini.accent { border-left-color: var(--secondary); align-items: center; text-align: center; }
    .spec-card-mini.accent:hover { border-color: rgba(0,229,255,0.4); box-shadow: 0 10px 25px rgba(0,229,255,0.2); }
    .spec-label-mini { font-size: 11px; color: #a0a0ab; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px; font-weight: 800; }
    ...
    @media (max-width: 1400px), (max-height: 850px) {
        ...
        .welcome-header-row img.side-logo { height: 75px; }
        .device-title { font-size: 26px; letter-spacing: 1.5px; padding: 0 15px; }
    ...
    @media (max-width: 1100px), (max-height: 700px) {
        .panel { padding: 20px 30px 20px 60px; }
        .montag-logo-main { height: 80px; }
        .sidebar { width: 230px; padding: 15px 0; }
        .sidebar img { width: 150px !important; }
        .nav-btn { padding: 12px 20px; font-size: 12px; }
        .header-system img.brand { height: 60px; }
        .header-system h1 { font-size: 22px; }
        .welcome-container { max-width: 100%; }
        .welcome-header-row { margin-bottom: 20px; }
        .welcome-header-row img.side-logo { height: 60px; }
        .device-title { font-size: 22px; letter-spacing: 1px; padding: 0 10px; }
        .spec-label-mini { font-size: 10px; margin-bottom: 4px; }
        .spec-value-mini { font-size: 14px; }
        .spec-card-mini { padding: 12px 15px; }
        .btn-action-pro { padding: 14px 40px; font-size: 13px; }
        .dash-grid { grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 15px; }
        .dash-card { padding: 20px; }
        .dash-card h3 { font-size: 17px; }
        .test-card-ultimate { padding: 25px; }
        .btn-hero { padding: 18px; font-size: 14px; margin-bottom: 15px; }
        .btn-grid { padding: 14px 10px; font-size: 11px; }
        .kb-frame { padding: 20px; border-radius: 20px; transform: scale(0.85); transform-origin: top center; }
    }
</style>
</head>
<body>
...
<div class="sidebar">
    ...
    <button class="nav-btn" id="nav-win" onclick="switchMainTab('tab-win', this, 'Windows Setup Engine')"><span>[2] Windows Setup</span></button>
    <button class="nav-btn" id="nav-drv" onclick="switchMainTab('tab-drv', this, 'Drivers Center')"><span>[3] Drivers Center</span></button>
    <button class="nav-btn" id="nav-sw" onclick="switchMainTab('tab-sw', this, 'Software Hub')"><span>[4] Software Hub</span></button>
    <div style="flex:1"></div>
    <button id="btnReport" class="btn-report-ultimate" onclick="requestReport()">Generate Final Report</button>
...
<div class="panel">
    ...
    <div id="tab-hw" class="section active">
        <div id="welcome-view" class="welcome-container">
            <div class="device-badge">TARGET DEVICE IDENTIFIED</div>
            
            <div class="welcome-header-row">
                <img src="$BrandLogo" alt="Brand" class="side-logo">
                <h2 class="device-title">$FullModel</h2>
                <img src="$CpuLogo" alt="CPU" class="side-logo">
            </div>
            
            <div class="spec-grid-custom">
                <div class="spec-card-mini accent">
                    <span class="spec-label-mini">Serial Number</span>
                    <div class="spec-value-mini" style="font-size: 22px; font-weight: 800; color: var(--secondary); letter-spacing: 2px;">$serialNum</div>
                </div>

                <div class="spec-card-mini">
                    <span class="spec-label-mini">Processor Engine (CPU)</span>
                    <div class="spec-value-mini">$cpuDetails</div>
                </div>

                <div class="spec-card-mini">
                    <span class="spec-label-mini">Graphics Processor (GPU)</span>
                    <div class="spec-value-mini" style="color: #e0e0e0;">$gpuString</div>
                </div>

                <div class="spec-row-split">
                    <div class="spec-card-mini">
                        <span class="spec-label-mini">Installed Memory (RAM)</span>
                        <div class="spec-value-mini">$ramDetails</div>
                    </div>
                    <div class="spec-card-mini">
                        <span class="spec-label-mini">Internal Storage</span>
                        <div class="spec-value-mini">$storageString</div>
                    </div>
                </div>
            </div>
        ...
        <div id="test-au" class="test-view"><button class="btn-back" onclick="closeTest()">BACK TO DASHBOARD</button><div class="test-card-ultimate"><h2 style="margin-bottom:20px;">Stereo Sound Engine</h2><p style="color:#666; margin-bottom:45px;">Verify left and right acoustic channels.</p><div style="display:flex; gap:25px; justify-content:center; margin-bottom:30px;"><button class="btn-action-pro" style="background:#3b82f6; margin-top:0;" onclick="playT(-1)">Left Channel</button><button class="btn-action-pro" style="background:#10b981; margin-top:0;" onclick="playT(1)">Right Channel</button></div><div style="margin-top:40px; border-top:1px solid rgba(255,255,255,0.05); padding-top:40px;"><h3 style="letter-spacing:3px; margin-bottom:30px;">AUDIO INTEGRITY?</h3><button class="btn-action-pro" style="background:var(--success); padding:20px 50px; margin-right:15px;" onclick="verifyAU('OK')">YES - CLEAR</button><button class="btn-action-pro" style="background:var(--error); padding:20px 50px;" onclick="verifyAU('X')">NO - DEFECT</button></div></div></div>
        ...
        <div id="test-stress" class="test-view">
            <button class="btn-back" onclick="closeTest()">BACK TO DASHBOARD</button>
            <div class="test-card-ultimate" style="max-width: 900px; padding: 50px;">
                <h2 style="margin-bottom: 30px; letter-spacing: 2px;">Extreme Performance Stress</h2>
                
                <div style="text-align:left; border-left:4px solid var(--secondary); padding-left:15px; margin-bottom:15px;">
                    <h3 style="margin:0; color:var(--secondary);">1. CPU (Processor Core Stress)</h3>
                    <p style="margin:5px 0 0 0; color:#777; font-size:12px;">Forces all logical cores to 100% via multi-threaded math calculations.</p>
                </div>
                <div class="btn-grid-container" style="margin-bottom:35px;">
                    <button class="btn-grid" onclick="runCmd('CPU_30')">30 Seconds</button>
                    <button class="btn-grid" onclick="runCmd('CPU_60')">60 Seconds</button>
                    <button class="btn-grid" style="border-color:#ff007f; color:#ff007f;" onclick="runCmd('CPU_INF')">Infinite Load</button>
                </div>

                <div style="text-align:left; border-left:4px solid var(--success); padding-left:15px; margin-bottom:15px;">
                    <h3 style="margin:0; color:var(--success);">2. RAM (Memory Allocation Load)</h3>
                    <p style="margin:5px 0 0 0; color:#777; font-size:12px;">Rapidly allocates large byte arrays to fill up physical memory.</p>
                </div>
                <div class="btn-grid-container" style="margin-bottom:35px;">
                    <button class="btn-grid" onclick="runCmd('RAM_30')">30 Seconds</button>
                    <button class="btn-grid" onclick="runCmd('RAM_60')">60 Seconds</button>
                    <button class="btn-grid" style="border-color:#ff007f; color:#ff007f;" onclick="runCmd('RAM_INF')">Infinite Load</button>
                </div>

                <div style="text-align:left; border-left:4px solid var(--primary); padding-left:15px; margin-bottom:15px;">
                    <h3 style="margin:0; color:var(--primary);">3. GPU (Graphics Render Load)</h3>
                    <p style="margin:5px 0 0 0; color:#777; font-size:12px;">Initiates Direct3D Engine to stress Dedicated/Integrated Graphics.</p>
                </div>
    ...
    <div id="tab-win" class="section">
        <div class="test-card-ultimate" style="max-width: 900px; padding: 50px;">
            ...
            <div class="btn-grid-container">
                ...
                <button class="btn-grid" onclick="runCmd('UPDATE')">Windows Update</button>
    ...
    <div id="tab-sw" class="section">
        <div class="test-card-ultimate" style="max-width: 900px; padding: 50px;">
            ...
            <div class="btn-grid-container">
                ...
                <button class="btn-grid" onclick="runCmd('APP_WINRAR')">Install WinRAR Pro</button>
                <button class="btn-grid" onclick="runCmd('APP_DEFCONT')">Install Defender Control</button>
                <button class="btn-grid" onclick="runCmd('APP_REVO')">Install Revo Uninstaller</button>
                <button class="btn-grid" onclick="runCmd('APP_GAMING')">Install Gaming Libraries</button>
                <button class="btn-grid" onclick="runCmd('APP_MAS')">MAS Activator Script</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function openTest(t) { document.getElementById('hw-dashboard').style.display = 'none'; document.querySelectorAll('.test-view').forEach(v => v.classList.remove('active')); document.getElementById(t).classList.add('active'); }
    ...
    window.addEventListener('keydown', e => { const k = document.getElementById(e.code); if(k && document.getElementById('test-kb').classList.contains('active')) { e.preventDefault(); k.classList.add('pressed'); pressedKeys.add(e.code); if(masterReq.filter(key => !pressedKeys.has(key)).length === 0) manualKBFinish(); } });
    ...
    function startTO() { 
        ...
        if(infoH1) {
            infoH1.innerText = 'TOUCH TEST';
            infoH1.style.color = '#fff';
        }
    ...
    window.addEventListener('touchmove', function(e) {
        if(document.getElementById('touch-surface-master').style.display === 'block') actTouch(e);
    }, {passive: false});

    window.addEventListener('mousemove', function(e) {
        if(document.getElementById('touch-surface-master').style.display === 'block' && e.buttons === 1) actTouch(e);
    });

    function finalizeTouch(st) { 
        document.getElementById('touch-surface-master').style.display = 'none'; 
        doneTests.to = true; 
        document.getElementById('c-stat-to').innerText = st; 
        document.getElementById('c-stat-to').className = st === "OK" ? "card-status ok" : "card-status"; 
        document.title = "MONTAG_TO_" + st; 
        checkHubStatus(); 
        closeTest(); 
    }
    
    document.addEventListener('fullscreenchange', () => { 
        if(!document.fullscreenElement && document.getElementById('touch-surface-master').style.display === 'block') { 
            let a = document.querySelectorAll('.touched').length;
            finalizeTouch(a >= 238 ? "OK" : "X"); 
        } 
    });
    ...
    function requestReport() { document.title = "MONTAG_CMD_REPORT"; setTimeout(() => window.close(), 1500); }
...
Start-Process "msedge" -ArgumentList "--new-window --start-maximized --app=`"$GuiFile`""
    ...
    if ($activeWins) {
        foreach ($e in $activeWins) {
            ...
            if ($title -match "MONTAG_CMD_(.+)") { 
                $cmd = $matches[1]
                if ($cmd -eq "REPORT") { "YES" | Out-File "$env:TEMP\action_report.txt" -Encoding ASCII; Stop-Process -Id $e.Id -Force -ErrorAction SilentlyContinue; break }
                if ($cmd -eq "EXIT") { Stop-Process -Id $e.Id -Force -ErrorAction SilentlyContinue; break }
